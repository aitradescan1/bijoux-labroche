  ...
  return send({
    ...
    html: layout(`
      ...
      <div class="card"><table>
        ...
        <tr><td>Courriel</td><td><a href="mailto:${email}">${email}</a></td></tr>
        <tr><td>Sujet</td><td>${subject}</td></tr>
      </table></div>
      <p style="white-space:pre-wrap;line-height:1.7">${message.replace(/</g,'&lt;')}</p>
      <a class="btn" href="mailto:${email}?subject=Re: ${encodeURIComponent(subject)}">Répondre →</a>
    `)
  });
}
