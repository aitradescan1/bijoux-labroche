...
function orderCard(o) {
  ...
  return `<div class="card"><table>
    ...
    ${o.rabais > 0 ? `<tr><td>Rabais 35 $+</td><td style="color:#2e7d32">−${Number(o.rabais).toFixed(2)} $CAD</td></tr>` : ''}
...
export function sendPaymentConfirmedAdmin(order) {
  return send({
    ...
    subject: `💰 Vente — ${order.product_name} · ${Number(order.amount_cad).toFixed(2)} $ (${order.customer_name})`,
    html: layout(`
...
// ════════════════════════════════════════════════════════════════
// 4. Message de contact — email à l'ADMIN
// ════════════════════════════════════════════════════════════════
function esc(s) {
...
export function sendContactMessage({ name, email, subject, message }) {
  return send({
    to:       process.env.EMAIL_DESTINATAIRE,
    replyTo:  email,
    subject:  `✉️ Contact site — ${subject}`,
    html: layout(`
      <h2>Nouveau message de contact</h2>
      <div class="card"><table>
        <tr><td>Nom</td><td><strong>${esc(name)}</strong></td></tr>
        <tr><td>Courriel</td><td><a href="mailto:${esc(email)}">${esc(email)}</a></td></tr>
        <tr><td>Sujet</td><td>${esc(subject)}</td></tr>
      </table></div>
      <p style="white-space:pre-wrap;line-height:1.7">${esc(message)}</p>
      <a class="btn" href="mailto:${esc(email)}?subject=Re%3A%20${encodeURIComponent(subject)}">Répondre →</a>
    `)
  });
}

// ════════════════════════════════════════════════════════════════
// 5. Commande prête — email au CLIENT
// ════════════════════════════════════════════════════════════════
