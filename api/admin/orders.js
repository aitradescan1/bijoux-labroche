...
// Query params optionnels: ?status=paid&limit=50&offset=0
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let query = supabase
      .from('orders')
      .select('id, customer_name, customer_email, product_name, qty, amount_cad, status, paypal_order_id, notes, created_at', { count: 'exact' })
    ...
    return res.status(200).json({ orders: data, total: count, limit, offset });
